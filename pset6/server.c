 ...
 */
char* indexes(const char* path)  //!indexes
{
    /* TODO Complete the implementation of indexes in such a way that the function,
    given a /path/to/a/directory, 
    returns /path/to/a/directory/index.php if index.php actually exists therein,
    or /path/to/a/directory/index.html if index.html actually exists therein,
    or NULL.
    In the first 2 of those cases, this function should dynamically allocate memory on the heap for the returned string.
    */
    
    //generate string for both lines
    char* pathCopy = malloc (LimitRequestLine);
    strcpy(pathCopy, path);

    char* php = "/index.php\0";
    strcpy( strrchr(pathCopy, '/') , php);
    
    if (access(pathCopy, R_OK) == 0)
        return pathCopy;
    
    char* html = "/index.html\0";
    strcpy( strrchr(pathCopy, '/') , html);
...
bool load(FILE* file, BYTE** content, size_t* length)                           // !LOAD !!!!!!!!!!!!!!!!!!!
{
    ...
    //allocate memory and prepare to load
    *content = malloc (1); 
    int size = 0;
    int byte = 0;
    byte = fgetc(file); 
    ...
    while (byte != EOF)
        {
            *(*content + size) = byte; //write to newest allocated byte
            *content = realloc(*content, (size + 2)); // reallocate memory
            size++;
            byte = fgetc(file);
        }
...
const char* lookup(const char* path)
{
    ...
    //copy from last '.' in path to type
   char* type = strrchr(path, '.');
    
    //compare type to all possible cases, irrespective of case
 ...
 */
bool parse(const char* line, char* abs_path, char* query)                           //!PARSE !!!!!!!!!!!!!!!!!!
{
